...
class HS:
    ...
    def hs_lists(self):
        self.heartstone_list = []
        self.heartstone_dic = {}
        url = 'http://bbs.ngacn.cc/thread.php?fid=422'
        r = requests.get(url, headers = self.h, timeout=5)
        r.encoding = 'gbk'
        bsh = BeautifulSoup(r.text, 'lxml')
        for i in bsh.findAll('td', class_ = 'c2'):
            self.heartstone_list.append(i.text.replace('\n', ''))
            self.heartstone_dic[i.text.replace('\n','')] = i.a.attrs['href']
        self.heartstone_dic = pd.DataFrame.from_dict(self.heartstone_dic, orient = 'index')
        self.heartstone_dic = self.heartstone_dic.reset_index(drop = False)
        self.heartstone_dic.columns = ['题目', '链接']
        return self.heartstone_list


    def read_hs(self, x, num):
        y = self.heartstone_dic.iloc[list(self.heartstone_dic['题目']).index(x), 1]
